...
contract('TokenFarm', ([owner, investor]) => {
  ...
  // Test deployment of Mock DAI Token
  ...
  describe('Mock Dapp Token deployment', async () => {
    it('has a name', async () => {
  ...
  describe('Farming tokens', async () => {
    it('rewards investors for staking mDai tokens', async () => {
      ...
      assert.equal(result.toString(), tokens('100'), 'Token Farm mDai wallet balance correct after staking');

      // Check Investor's staking balance after staking
      result = await tokenFarm.stakingBalance(investor);
      assert.equal(result.toString(), tokens('100'), 'Investor staking balance correct after staking');

      // Check that investor is currently staking
      ...
      assert.equal(result.toString(), tokens('100'), 'Investor Dapp Token wallet balance cerrect after issuance');

      // Ensure that the owner is the only once who can call the issueTokens() function
      ...
      result = await daiToken.balanceOf(tokenFarm.address);
      assert.equal(result.toString(), tokens('0'), 'Token Farm mDai balance correct after unstaking');
